...
<details>
<summary><strong>Dokploy MCP — Deploy & Manage via Claude</strong></summary>

Enables the [Dokploy](https://dokploy.com) MCP server, launched as a local stdio subprocess via `npx`. When active, Claude can interact with your Dokploy deployment platform directly from the chat (list apps, check status, trigger deploys, etc.).

**Requirements:** `npx` available in `PATH` and a running Dokploy instance with an API key.

```bash
# .env
DOKPLOY_MCP_ENABLED=true
DOKPLOY_URL=https://your-dokploy-server.com/api
DOKPLOY_API_KEY=your-dokploy-api-token
```

When `DOKPLOY_MCP_ENABLED=false` (the default), no Dokploy process is started and no tools are registered.
